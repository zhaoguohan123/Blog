...
DWORD ProcessInfo(LPWSTR PName)
{
...
int wmain(int argc, LPWSTR * argv)
{
    ...
    DWORD PID = ProcessInfo(argv[1]);
    DWORD TID = GetThreadInfo(PID);
    ...
    if (hThread != NULL)
    {
        ...
        if (FAddr != NULL)
        {
            if (WriteProcessMemory(hProcess, FAddr, DllPath, dwSize, NULL))
            {
                ...
                if (ProcH == NULL)
                {
                    std::cout << "ProcH is NULL!" << std::endl;
                    
                }
                if (!QueueUserAPC((PAPCFUNC)ProcH, hThread,(ULONG_PTR)FAddr))
                {
                    std::cout << "QueueUserAPC Failed! errcode: " << GetLastError() <<std::endl;
                }
                else
                {
